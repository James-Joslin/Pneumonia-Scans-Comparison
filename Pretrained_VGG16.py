...
pretrained_model = vgg(inputs, training=False)
...
lr_sched = step_decay_schedule(initial_lr=1e-3, decay_factor=0.75, step_size=4)
history = model.fit(
    train_data,
...
save_model(model_In=model, save_name="Pretrained_VGG16")
modelAccuracy(data = test_data, model_In=model, save_name="VGG16_Performance", time = Duration)
